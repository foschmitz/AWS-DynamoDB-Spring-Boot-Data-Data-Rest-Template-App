 ...
 * .setLoginEndpoint(TEST_URL + EstateSvcApi.TOKEN_PATH) .setUsername(USERNAME)
...
public class SecuredRestBuilder extends RestAdapter.Builder {
	...
	private class OAuthHandler implements RequestInterceptor {
		...
		public void intercept(RequestFacade request) {
			...
			if (!loggedIn) {
				try {
					// This code below programmatically builds an OAuth 2.0
					// password
					...
					// client.
					// In order to do BASIC authentication, we need to
					// concatenate the
					// client_id and client_secret values together with a colon
					// and then
					// Base64 encode them. The final value is added to the
					// request as
					// the "Authorization" header and the value is set to
					// "Basic "
					// concatenated with the Base64 client_id:client_secret
					// value described
					// above.
					...
					headers.add(new Header("Authorization", "Basic "
							+ base64Auth));

					// Create the actual password grant request using the data
					// above
					...
					if (resp.getStatus() < 200 || resp.getStatus() > 299) {
						...
						// Extract the access_token (bearer token) from the
						// response so that we
						...
						// Let future calls know we've already fetched the
						// access token
						loggedIn = true;
					}
				} catch (Exception e) {
	...
	@Override
	public SecuredRestBuilder setProfiler(
			@SuppressWarnings("rawtypes") Profiler profiler) {
	...
	public RestAdapter build() {
		...
		OAuthHandler hdlr = new OAuthHandler(client, loginUrl, username,
